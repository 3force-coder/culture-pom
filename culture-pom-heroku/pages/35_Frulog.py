...
def get_combinaisons_non_mappees_produit():
    try:
        conn = get_connection(); cursor = conn.cursor()
        cursor.execute("""SELECT fl.emballage, fl.marque, COUNT(*) as nb_lignes,
                   SUM(ABS(COALESCE(fl.pds_net, 0))) as pds_total_kg,
                   COUNT(DISTINCT fl.client) as nb_clients
            FROM frulog_lignes_condi fl
            WHERE fl.type = 'E' AND fl.code_produit_commercial IS NULL AND fl.emballage IS NOT NULL
...
def get_emballages_non_mappes_se():
    try:
        conn = get_connection(); cursor = conn.cursor()
        cursor.execute("""SELECT fl.emballage as code_emballage, COUNT(*) as nb_lignes,
                   SUM(COALESCE(fl.nb_col, 0)) as nb_col_total
            FROM frulog_lignes_condi fl
            WHERE fl.type = 'E' AND fl.sur_emballage_id IS NULL AND fl.emballage IS NOT NULL
...
def modifier_mapping_produit(mapping_id, code_produit, description=None):
    try:
        conn = get_connection(); cursor = conn.cursor()
        cursor.execute("""UPDATE frulog_mapping_produit SET code_produit_commercial=%s, description=%s, updated_at=CURRENT_TIMESTAMP
            WHERE id=%s RETURNING emballage, marque""", (code_produit, description, mapping_id))
        row = cursor.fetchone()
        if row:
            cursor.execute("UPDATE frulog_lignes_condi SET code_produit_commercial=%s WHERE emballage=%s AND (marque=%s OR (%s IS NULL AND marque IS NULL))",
...
# ============================================================================
# IMPORT GÉNÉRIQUE — Ventes (CONDI / NEGOCE)
# ============================================================================

def importer_ventes(uploaded_file, source, username='inconnu'):
    """Import upsert pour CONDI ou NEGOCE"""
    ...
    try:
        ...
        if 'no_de_bon' not in df_c.columns:
            return False, "Colonne 'No de bon' introuvable"

        # Filtrer E et A seulement
        ...
        if source == 'CONDI':
            data_cols += ['code_produit_commercial', 'sur_emballage_id']
        ...
        for _, row in df_c.iterrows():
            ...
            if bon in existing:
                ex = existing[bon]
                changed = any(str(to_python(row.get(k))) != str(ex.get(k)) for k in compare_keys)
                if changed:
                    set_parts = [f"{col}=%s" for col in data_cols] + ["import_id=%s"]
                    vals = [row_values[col] for col in data_cols] + [import_id, bon]
                    cursor.execute(f"UPDATE {table} SET {', '.join(set_parts)} WHERE no_de_bon=%s", vals)
                    nb_updated += 1
                else:
                    nb_unchanged += 1
            else:
                ...
                cursor.execute(f"INSERT INTO {table} ({', '.join(all_cols)}) VALUES ({ph})", all_vals)
                nb_new += 1

        conn.commit()
        nb_mapped = df_c['code_produit_commercial'].notna().sum() if 'code_produit_commercial' in df_c.columns else 0
        cursor.close(); conn.close()
        map_msg = f" {nb_mapped}/{nb_total} mappées." if source == 'CONDI' else ""
        return True, f"Import #{import_id} ({source}) : {nb_total} lignes → **{nb_new} nouvelles**, **{nb_updated} modifiées**, {nb_unchanged} inchangées.{map_msg}"
    except Exception as e:
        if 'conn' in locals(): conn.rollback()
        ...
        for _, row in df_c.iterrows():
            ...
            if bon in existing:
                ...
                if changed:
                    set_parts = [f"{col}=%s" for col in data_cols] + ["import_id=%s"]
                    vals = [row_values[col] for col in data_cols] + [import_id, bon]
                    cursor.execute(f"UPDATE frulog_lignes_achat SET {', '.join(set_parts)} WHERE no_de_bon=%s", vals)
                    nb_updated += 1
                else:
                    nb_unchanged += 1
            else:
                ...
                all_vals = [import_id, bon] + [row_values[col] for col in data_cols]
                ph = ', '.join(['%s'] * len(all_cols))
                cursor.execute(f"INSERT INTO frulog_lignes_achat ({', '.join(all_cols)}) VALUES ({ph})", all_vals)
                nb_new += 1
...
def get_analyse_ventes(table):
    """Analyse complète d'une table ventes (condi ou negoce)"""
    try:
        conn = get_connection(); cursor = conn.cursor()
        r = {}
        ...
        for key, sql in [
            ('par_client', f"SELECT client, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca, AVG(prix) FILTER (WHERE prix>0) as prix_moy, MIN(date_charg) as premiere, MAX(date_charg) as derniere FROM {table} WHERE type='E' GROUP BY client ORDER BY pds_kg DESC"),
            ('par_variete', f"SELECT variete, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca, COUNT(DISTINCT client) as nb_clients, AVG(prix) FILTER (WHERE prix>0) as prix_moy FROM {table} WHERE type='E' AND variete IS NOT NULL GROUP BY variete ORDER BY pds_kg DESC"),
            ('par_mois', f"SELECT EXTRACT(YEAR FROM date_charg)::int as annee, EXTRACT(MONTH FROM date_charg)::int as mois, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca FROM {table} WHERE type='E' AND date_charg IS NOT NULL GROUP BY 1,2 ORDER BY annee, mois"),
            ('par_semaine', f"SELECT annee, semaine, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca, COUNT(DISTINCT client) as nb_clients FROM {table} WHERE type='E' AND annee IS NOT NULL GROUP BY annee, semaine ORDER BY annee, semaine"),
            ('par_produit', f"SELECT COALESCE(code_produit_commercial, '❓ Non mappé') as produit, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca FROM {table} WHERE type='E' GROUP BY 1 ORDER BY pds_kg DESC"),
            ('par_emballage', f"SELECT emballage, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg FROM {table} WHERE type='E' AND emballage IS NOT NULL GROUP BY emballage ORDER BY pds_kg DESC"),
            ('par_calibre', f"SELECT calibre, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, COUNT(DISTINCT client) as nb_clients FROM {table} WHERE type='E' AND calibre IS NOT NULL GROUP BY calibre ORDER BY pds_kg DESC"),
            ('par_vendeur', f"SELECT vendeur, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca, COUNT(DISTINCT client) as nb_clients FROM {table} WHERE type='E' AND vendeur IS NOT NULL GROUP BY vendeur ORDER BY ca DESC"),
            ('par_annee', f"SELECT EXTRACT(YEAR FROM date_charg)::int as annee, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant,0)) as ca, COUNT(DISTINCT client) as nb_clients, COUNT(*) as nb FROM {table} WHERE type='E' AND date_charg IS NOT NULL GROUP BY 1 ORDER BY annee"),
        ]:
            cursor.execute(sql)
            r[key] = cursor.fetchall()
        cursor.close(); conn.close()
        return r
    except Exception as e:
...
def get_analyse_achat():
    """Analyse de la table achats"""
    try:
        conn = get_connection(); cursor = conn.cursor()
        r = {}
        cursor.execute("""SELECT COUNT(*) as total, COUNT(DISTINCT apporteur) as nb_fournisseurs,
        ...
        for key, sql in [
            ('par_fournisseur', "SELECT apporteur, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant_euro,0)) as ca, AVG(prix_achat) FILTER (WHERE prix_achat>0) as prix_moy, COUNT(DISTINCT variete) as nb_varietes FROM frulog_lignes_achat GROUP BY apporteur ORDER BY pds_kg DESC"),
            ('par_variete', "SELECT variete, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant_euro,0)) as ca, COUNT(DISTINCT apporteur) as nb_fournisseurs, AVG(prix_achat) FILTER (WHERE prix_achat>0) as prix_moy FROM frulog_lignes_achat WHERE variete IS NOT NULL GROUP BY variete ORDER BY pds_kg DESC"),
            ('par_mois', "SELECT EXTRACT(YEAR FROM dt_chargmt)::int as annee, EXTRACT(MONTH FROM dt_chargmt)::int as mois, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant_euro,0)) as ca FROM frulog_lignes_achat WHERE dt_chargmt IS NOT NULL GROUP BY 1,2 ORDER BY annee, mois"),
            ('par_depot', "SELECT depot, COUNT(*) as nb, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant_euro,0)) as ca FROM frulog_lignes_achat WHERE depot IS NOT NULL GROUP BY depot ORDER BY pds_kg DESC"),
            ('par_annee', "SELECT EXTRACT(YEAR FROM dt_chargmt)::int as annee, SUM(COALESCE(pds_net,0)) as pds_kg, SUM(COALESCE(montant_euro,0)) as ca, COUNT(DISTINCT apporteur) as nb_fournisseurs, COUNT(*) as nb FROM frulog_lignes_achat WHERE dt_chargmt IS NOT NULL GROUP BY 1 ORDER BY annee"),
        ]:
            cursor.execute(sql)
            r[key] = cursor.fetchall()
        cursor.close(); conn.close()
...
def get_comparaison_previsions():
    try:
        conn = get_connection(); cursor = conn.cursor()
        cursor.execute("""WITH expedie AS (
                SELECT code_produit_commercial, annee, semaine, SUM(pds_net)/1000.0 as tonnes_exp
                FROM frulog_lignes_condi WHERE type='E' AND code_produit_commercial IS NOT NULL
...
def render_analyse_ventes(data, label, show_produit=False):
    """Rendu complet d'analyse pour ventes (CONDI ou NEGOCE)"""
    if not data or not data.get('kpis'):
        st.info(f"📭 Aucune donnée {label}")
        return
    k = data['kpis']
    if not k or k['total'] == 0:
        st.info(f"📭 Aucune donnée {label}")
        return
    ...
    if vue == "📊 Vue d'ensemble":
        ...
        if data['par_mois']:
            ...
            fig = go.Figure([go.Scatter(x=df_m['label'], y=df_m['tonnes'], mode='lines+markers',
        ...
        # Hebdo 52 dernières semaines
        if data['par_semaine']:
            df_s = pd.DataFrame(data['par_semaine']).tail(52)
            ...
            fig.update_layout(title="Expéditions / semaine — 12 derniers mois (T)", height=400, xaxis=dict(tickangle=-45))
            st.plotly_chart(fig, use_container_width=True)
        ...
        with gc1:
            fig = px.bar(df.head(20), x='client', y='tonnes', title="Top 20 Clients (T)",
                        color='tonnes', color_continuous_scale='blues')
            fig.update_layout(height=450, xaxis_tickangle=-45, showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        with gc2:
            fig = px.bar(df.head(20), x='client', y='ca_k', title="Top 20 Clients (k€)",
                        color='ca_k', color_continuous_scale='greens')
            fig.update_layout(height=450, xaxis_tickangle=-45, showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        # Pareto
        df_s = df.sort_values('ca', ascending=False).copy()
        df_s['pct_cumul'] = df_s['ca'].astype(float).cumsum() / df_s['ca'].astype(float).sum() * 100
        ...
        st.dataframe(df[['client','tonnes','ca_k','prix_moy','premiere','derniere']].head(50).rename(columns={
            'client':'Client','tonnes':'Tonnes','ca_k':'CA (k€)','prix_moy':'Prix moy (€/T)',
            'premiere':'1ère expéd.','derniere':'Dernière'
        ...
        st.download_button(f"📥 Export clients {label}", buf.getvalue(),
                          f"clients_{label}_{datetime.now().strftime('%Y%m%d')}.xlsx", use_container_width=True)
        ...
        with gv1:
            fig = px.pie(df.head(15), names='variete', values='tonnes', title="Top 15 Variétés (T)")
            fig.update_layout(height=450); st.plotly_chart(fig, use_container_width=True)
        with gv2:
            fig = px.bar(df.head(15), x='variete', y='prix_moy', title="Prix moy / variété (€/T)",
                        color='prix_moy', color_continuous_scale='YlOrRd')
        ...
        st.dataframe(df[['variete','tonnes','nb_clients','prix_moy','nb']].head(30).rename(columns={
            'variete':'Variété','tonnes':'Tonnes','nb_clients':'Clients','prix_moy':'Prix moy (€/T)','nb':'Lignes'
        }), use_container_width=True, hide_index=True,
            column_config={'Prix moy (€/T)': st.column_config.NumberColumn(format="%.0f")})
        ...
        df['tonnes'] = df['pds_kg'].astype(float) / 1000
        df['ca_k'] = df['ca'].astype(float) / 1000
        fig = px.bar(df, x='produit', y='tonnes', title="Tonnage / produit commercial",
                    color='tonnes', color_continuous_scale='blues')
        fig.update_layout(height=400, xaxis_tickangle=-45, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['produit','tonnes','ca_k','nb']].rename(columns={
            'produit':'Produit','tonnes':'Tonnes','ca_k':'CA (k€)','nb':'Lignes'
        }), use_container_width=True, hide_index=True)

    elif vue == "📦 Emballages" and data.get('par_emballage'):
        df = pd.DataFrame(data['par_emballage'])
        df['tonnes'] = df['pds_kg'].astype(float) / 1000
        fig = px.pie(df.head(10), names='emballage', values='tonnes', title="Répartition emballages (T)")
        fig.update_layout(height=400); st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['emballage','tonnes','nb']].rename(columns={
            'emballage':'Emballage','tonnes':'Tonnes','nb':'Lignes'
        }), use_container_width=True, hide_index=True)

    elif vue == "📏 Calibres" and data.get('par_calibre'):
        df = pd.DataFrame(data['par_calibre'])
        df['tonnes'] = df['pds_kg'].astype(float) / 1000
        fig = px.bar(df, x='calibre', y='tonnes', title="Tonnage / calibre",
                    color='tonnes', color_continuous_scale='purples')
        fig.update_layout(height=400, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['calibre','tonnes','nb_clients','nb']].rename(columns={
            'calibre':'Calibre','tonnes':'Tonnes','nb_clients':'Clients','nb':'Lignes'
        }), use_container_width=True, hide_index=True)
        ...
        df['tonnes'] = df['pds_kg'].astype(float) / 1000
        df['ca_k'] = df['ca'].astype(float) / 1000
        fig = px.bar(df, x='vendeur', y='ca_k', title="CA / vendeur (k€)",
                    color='ca_k', color_continuous_scale='greens')
        fig.update_layout(height=400, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['vendeur','tonnes','ca_k','nb_clients']].rename(columns={
            'vendeur':'Vendeur','tonnes':'Tonnes','ca_k':'CA (k€)','nb_clients':'Clients'
        }), use_container_width=True, hide_index=True)


def render_analyse_achat(data):
    """Rendu analyse achats"""
    if not data or not data.get('kpis'):
        st.info("📭 Aucune donnée Achat")
        return
    k = data['kpis']
    if not k or k['total'] == 0:
        st.info("📭 Aucune donnée Achat")
    ...
    st.markdown("---")
    vue = st.radio("Analyser par :", ["📊 Vue d'ensemble", "🏭 Fournisseurs", "🥔 Variétés", "🏢 Dépôts"],
                   horizontal=True, key="vue_achat")
    st.markdown("---")
    ...
    if vue == "📊 Vue d'ensemble":
        if data['par_annee']:
            ...
            with ga1:
                ...
                fig.update_layout(title="Tonnage achat / année", height=350); st.plotly_chart(fig, use_container_width=True)
            with ga2:
                fig = go.Figure([go.Bar(x=df_a['annee'].astype(str), y=df_a['ca_k'], marker_color='#BF360C')])
                fig.update_layout(title="Montant achat / année (k€)", height=350); st.plotly_chart(fig, use_container_width=True)
        if data['par_mois']:
            df_m = pd.DataFrame(data['par_mois'])
            df_m['tonnes'] = df_m['pds_kg'].astype(float) / 1000
            df_m['label'] = df_m.apply(lambda r: f"{int(r['mois']):02d}/{int(r['annee'])}", axis=1)
            ...
            fig.update_layout(title="Tonnage achat mensuel", height=400, xaxis=dict(tickangle=-45))
            st.plotly_chart(fig, use_container_width=True)

    elif vue == "🏭 Fournisseurs" and data.get('par_fournisseur'):
        df = pd.DataFrame(data['par_fournisseur'])
        df['tonnes'] = df['pds_kg'].astype(float) / 1000
        df['ca_k'] = df['ca'].astype(float) / 1000
        st.markdown(f"##### {len(df)} fournisseurs")
        gf1, gf2 = st.columns(2)
        with gf1:
            fig = px.bar(df.head(20), x='apporteur', y='tonnes', title="Top 20 Fournisseurs (T)",
                        color='tonnes', color_continuous_scale='oranges')
            fig.update_layout(height=450, xaxis_tickangle=-45, showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        with gf2:
            fig = px.bar(df.head(20), x='apporteur', y='prix_moy', title="Prix achat moy (€/T)",
                        color='prix_moy', color_continuous_scale='YlOrRd')
            fig.update_layout(height=450, xaxis_tickangle=-45, showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['apporteur','tonnes','ca_k','prix_moy','nb_varietes','nb']].head(50).rename(columns={
            'apporteur':'Fournisseur','tonnes':'Tonnes','ca_k':'Montant (k€)',
            'prix_moy':'Prix moy (€/T)','nb_varietes':'Variétés','nb':'Lignes'
        }), use_container_width=True, hide_index=True,
            column_config={'Prix moy (€/T)': st.column_config.NumberColumn(format="%.0f")})
        buf = io.BytesIO(); df.to_excel(buf, index=False, engine='openpyxl')
        st.download_button("📥 Export fournisseurs", buf.getvalue(),
                          f"fournisseurs_achat_{datetime.now().strftime('%Y%m%d')}.xlsx", use_container_width=True)
        ...
        gv1, gv2 = st.columns(2)
        with gv1:
            fig = px.pie(df.head(15), names='variete', values='tonnes', title="Top 15 Variétés achetées (T)")
            fig.update_layout(height=450); st.plotly_chart(fig, use_container_width=True)
        with gv2:
            fig = px.bar(df.head(15), x='variete', y='prix_moy', title="Prix achat moy / variété",
                        color='prix_moy', color_continuous_scale='YlOrRd')
            fig.update_layout(height=450, showlegend=False); st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['variete','tonnes','nb_fournisseurs','prix_moy','nb']].head(30).rename(columns={
            'variete':'Variété','tonnes':'Tonnes','nb_fournisseurs':'Fournisseurs','prix_moy':'Prix moy (€/T)','nb':'Lignes'
        }), use_container_width=True, hide_index=True,
            column_config={'Prix moy (€/T)': st.column_config.NumberColumn(format="%.0f")})
        ...
        fig = px.bar(df.head(20), x='depot', y='tonnes', title="Top 20 Dépôts (T)",
                    color='tonnes', color_continuous_scale='oranges')
        fig.update_layout(height=400, xaxis_tickangle=-45, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df[['depot','tonnes','ca_k','nb']].head(30).rename(columns={
            'depot':'Dépôt','tonnes':'Tonnes','ca_k':'Montant (k€)','nb':'Lignes'
        }), use_container_width=True, hide_index=True)
...
with tab1:
    imp_type = st.radio("Type d'import :", ["📈 Ventes Condi", "🏪 Ventes Négoce", "🛒 Achats"],
                        horizontal=True, key="imp_type")
    st.markdown("---")
    ...
    if imp_type == "📈 Ventes Condi":
        ...
        if up:
            try:
                df_preview = pd.read_excel(up, sheet_name=0, nrows=5)
            ...
            except Exception as e: st.error(f"Erreur lecture : {str(e)}")
        ...
        if up:
            try:
                up2 = up; up2.seek(0); df_count = pd.read_excel(up2, sheet_name=0)
                nb_e = len(df_count[df_count['Type']=='E']) if 'Type' in df_count.columns else 0
                c1, c2 = st.columns(2)
                with c1: st.metric("Lignes total", len(df_count))
                with c2: st.metric("Expéditions (E)", nb_e)
    ...
    df_imp = get_imports()
    if not df_imp.empty:
        st.dataframe(df_imp[['id','source','nom_fichier','date_import','nb_lignes_total',
...
# ============================================================================
# TAB 2 : MAPPING PRODUITS (CONDI)
# ============================================================================

with tab2:
    st.subheader("🔗 Mapping Emballage + Marque → Produit Commercial")
    st.caption("*Applicable aux données Condi uniquement*")
...
# ============================================================================
# TAB 3 : MAPPING SUR-EMBALLAGES (CONDI)
# ============================================================================
...
with tab3:
    ...
    if not df_nm_se.empty:
        ...
        st.dataframe(df_nm_se.rename(columns={
            'code_emballage':'Code Emballage','nb_lignes':'Nb lignes','nb_col_total':'Nb colis'
        }), use_container_width=True, hide_index=True)
        ...
        if sur_embs:
            ...
            with se1:
                ...
                sel_code = st.selectbox("Code emballage", codes, key="mse_code")
            with se2:
            ...
            if st.button("💾 Enregistrer", type="primary", key="btn_mse"):
                ok, msg = sauver_mapping_suremballage(sel_code, int(sur_embs[se_idx]['id']))
                if ok: st.success(f"✅ {msg}"); st.rerun()
                else: st.error(f"❌ {msg}")
    else:
        st.success("✅ Tous les codes sont associés")
...
# ============================================================================
# TAB 4 : CONDI — Analyse + Prévu vs Réel
# ============================================================================

with tab4:
    st.subheader("📈 Analyse Conditionnement")

    sous_onglet = st.radio("", ["📊 Analyse", "🎯 Prévu vs Réel"], horizontal=True, key="condi_sous")
    st.markdown("---")

    if sous_onglet == "📊 Analyse":
        ...
        if not df_comp.empty:
            ...
            taux_g = (df_avec['expedie_t'].astype(float).sum() / df_avec['prevu_t'].astype(float).sum() * 100) if not df_avec.empty else 0
            tot_p = df_comp['prevu_t'].astype(float).sum()
            tot_e = df_comp['expedie_t'].astype(float).sum()
            ...
            if df_sem['taux'].notna().any():
                ...
                fig_t.update_layout(title="Taux de réalisation / semaine (%)", height=350)
                st.plotly_chart(fig_t, use_container_width=True)

            st.markdown("---")
            df_pr = df_comp.groupby('produit').agg(
                prevu=('prevu_t', lambda x: x.astype(float).sum()),
                expedie=('expedie_t', lambda x: x.astype(float).sum())).reset_index()
            df_pr['ecart'] = df_pr['expedie'] - df_pr['prevu']
            df_pr['taux'] = df_pr.apply(lambda r: f"{r['expedie']/r['prevu']*100:.0f}%" if r['prevu']>0 else "—", axis=1)
            st.dataframe(df_pr.sort_values('prevu', ascending=False).rename(columns={
                'produit':'Produit','prevu':'Prévu (T)','expedie':'Expédié (T)','ecart':'Écart (T)','taux':'Taux'
            }), use_container_width=True, hide_index=True)
            ...
            with pd.ExcelWriter(buf, engine='openpyxl') as w:
                ...
                df_pr.to_excel(w, index=False, sheet_name='Par Produit')
            st.download_button("📥 Export Excel", buf.getvalue(),
                              f"prevu_vs_reel_{datetime.now().strftime('%Y%m%d')}.xlsx", use_container_width=True)
        else:
            st.info("📭 Aucune donnée. Importez des données Condi et/ou saisissez des prévisions.")

# ============================================================================
# TAB 5 : NÉGOCE — Analyse
# ============================================================================
