    ...
    if job_en_terminaison and job_elem_data is not None:
        ...
        with col_lave:
            ...
            with st.expander("🔢 Calculer depuis pesées individuelles"):
                ...
                # ⭐ Pallox partiel (dernier pallox non plein) — exclu de la moyenne (Q1=A, Q2=C)
                ...
                if pesees_lave:
                    ...
                    nb_echantillon = len(pesees_lave)
                    has_partiel = ajouter_partiel and poids_partiel_lave > 0
                    # ⭐ Q1=A échantillon : moyenne extrapolée sur le Nb Pallox du haut
                    # Q2=C : si partiel, les pleins = Nb Pallox - 1, + poids partiel
                    if has_partiel:
                        nb_pleins = max(0, nb_pallox_lave - 1)
                        poids_moy_lave_calc = (moy_lave * nb_pleins) + poids_partiel_lave
                    else:
                        nb_pleins = nb_pallox_lave
                        poids_moy_lave_calc = moy_lave * nb_pallox_lave

                    if has_partiel:
                        col_pm1, col_pm2, col_pm3, col_pm4 = st.columns(4)
                        col_pm1.metric("Échantillon", f"{nb_echantillon} pesées")
                        col_pm2.metric("Moy. plein", f"{moy_lave:,.0f} kg")
                        col_pm3.metric(f"{nb_pleins} pleins + partiel", f"{poids_partiel_lave:,.0f} kg")
                        col_pm4.metric(f"→ Total ({nb_pallox_lave}p)", f"{poids_moy_lave_calc:,.0f} kg")
                    else:
                    ...
                    # Garde-fou : prévenir si Nb Pallox = 0 (rien à extrapoler)
                    if nb_pallox_lave <= 0:
                        st.warning("⚠️ Renseigne d'abord le « Nb Pallox » en haut pour extrapoler la moyenne.")
                    elif has_partiel and nb_pallox_lave == 1:
                        st.info("ℹ️ 1 seul pallox = le pallox partiel uniquement (pas de plein).")
                else:
            ...
            st.metric("⚖️ Poids retenu", f"{poids_lave_auto:,.0f} kg",
                help="Calcul depuis échantillon (extrapolé + partiel éventuel)" if pesees_lave else f"{nb_pallox_lave} × {poids_unit_lave} kg")
